...
public partial class RhRf69
{
    ...
    // Register names
    private const byte REG_00_FIFO = 0x00;
    internal const byte REG_01_OPMODE = 0x01;
    private const byte REG_02_DATAMODUL = 0x02;
    private const byte REG_03_BITRATEMSB = 0x03;
    private const byte REG_04_BITRATELSB = 0x04;
    private const byte REG_05_FDEVMSB = 0x05;
    private const byte REG_06_FDEVLSB = 0x06;
    private const byte REG_07_FRFMSB = 0x07;
    private const byte REG_08_FRFMID = 0x08;
    private const byte REG_09_FRFLSB = 0x09;
    private const byte REG_0A_OSC1 = 0x0a;
    private const byte REG_0B_AFCCTRL = 0x0b;
    private const byte REG_0C_RESERVED = 0x0c;
    private const byte REG_0D_LISTEN1 = 0x0d;
    private const byte REG_0E_LISTEN2 = 0x0e;
    private const byte REG_0F_LISTEN3 = 0x0f;
    private const byte REG_10_VERSION = 0x10;
    private const byte REG_11_PALEVEL = 0x11;
    private const byte REG_12_PARAMP = 0x12;
    private const byte REG_13_OCP = 0x13;
    private const byte REG_14_RESERVED = 0x14;
    private const byte REG_15_RESERVED = 0x15;
    private const byte REG_16_RESERVED = 0x16;
    private const byte REG_17_RESERVED = 0x17;
    private const byte REG_18_LNA = 0x18;
    private const byte REG_19_RXBW = 0x19;
    private const byte REG_1A_AFCBW = 0x1a;
    private const byte REG_1B_OOKPEAK = 0x1b;
    private const byte REG_1C_OOKAVG = 0x1c;
    private const byte REG_1D_OOKFIX = 0x1d;
    private const byte REG_1E_AFCFEI = 0x1e;
    private const byte REG_1F_AFCMSB = 0x1f;
    private const byte REG_20_AFCLSB = 0x20;
    private const byte REG_21_FEIMSB = 0x21;
    private const byte REG_22_FEILSB = 0x22;
    private const byte REG_23_RSSICONFIG = 0x23;
    private const byte REG_24_RSSIVALUE = 0x24;
    internal const byte REG_25_DIOMAPPING1 = 0x25;
    private const byte REG_26_DIOMAPPING2 = 0x26;
    internal const byte REG_27_IRQFLAGS1 = 0x27;
    private const byte REG_28_IRQFLAGS2 = 0x28;
    private const byte REG_29_RSSITHRESH = 0x29;
    private const byte REG_2A_RXTIMEOUT1 = 0x2a;
    private const byte REG_2B_RXTIMEOUT2 = 0x2b;
    private const byte REG_2C_PREAMBLEMSB = 0x2c;
    private const byte REG_2D_PREAMBLELSB = 0x2d;
    private const byte REG_2E_SYNCCONFIG = 0x2e;
    private const byte REG_2F_SYNCVALUE1 = 0x2f;
    // another 7 sync word bytes follow, 30 through 36 inclusive
    private const byte REG_37_PACKETCONFIG1 = 0x37;
    private const byte REG_38_PAYLOADLENGTH = 0x38;
    private const byte REG_39_NODEADRS = 0x39;
    private const byte REG_3A_BROADCASTADRS = 0x3a;
    private const byte REG_3B_AUTOMODES = 0x3b;
    private const byte REG_3C_FIFOTHRESH = 0x3c;
    private const byte REG_3D_PACKETCONFIG2 = 0x3d;
    private const byte REG_3E_AESKEY1 = 0x3e;
    // Another 15 AES key bytes follow
    private const byte REG_4E_TEMP1 = 0x4e;
    private const byte REG_4F_TEMP2 = 0x4f;
    private const byte REG_58_TESTLNA = 0x58;
    private const byte REG_5A_TESTPA1 = 0x5a;
    private const byte REG_5C_TESTPA2 = 0x5c;
    private const byte REG_6F_TESTDAGC = 0x6f;
    private const byte REG_71_TESTAFC = 0x71;
